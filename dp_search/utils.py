...
def accuracy(output, target, topk=(1,)):
    ...
    _, pred = output.topk(maxk, 1, True, True)  # pred is the lower index, doesn't have gradient, requir_grads set ot be False here.
    pred = pred.t()
...
def _data_transforms_cifar10(args):
    ...
    # # (0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010) for cifar-100
    #
...
def arch_to_genotype(arch_normal, arch_reduce, n_nodes, cell_type, private=False):
    if cell_type == "SAMPLE":
...
def infinite_get(data_iter, data_queue):
    try:
        ...
        data_iter = iter(data_queue)  # do it again. # TODO: make sure data_iter and data_queue is from the same dataset
        data = next(data_iter)
